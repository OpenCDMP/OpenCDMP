...
@Service(TenantEntityManagerImpl.QualifierName)
@Qualifier(TenantEntityManagerImpl.QualifierName)
@RequestScope
public class TenantEntityManagerImpl implements TenantEntityManager {
	public static final String QualifierName = "tenantEntityManagerImpl";
	@PersistenceContext
	...
	public TenantEntityManagerImpl(TenantScopeFactory tenantScopeFactory, ErrorThesaurusProperties errors) {
	...
	public <T> T merge(T entity) throws InvalidApplicationException {
		if (!this.tenantFiltersDisabled && this.tenantScopeFactory.getInstance().isMultitenant() && (entity instanceof TenantScoped tenantScopedEntity)) {
			if (!this.tenantScopeFactory.getInstance().isDefaultTenant()) {
				if (tenantScopedEntity.getTenantId() == null || !tenantScopedEntity.getTenantId().equals(this.tenantScopeFactory.getInstance().getTenant())) throw new MyForbiddenException(this.errors.getTenantTampering().getCode(), this.errors.getTenantTampering().getMessage());
	...
	public <T> T find(Class<T> entityClass, Object primaryKey) throws InvalidApplicationException {
		...
		if (!this.tenantFiltersDisabled && this.tenantScopeFactory.getInstance().isMultitenant() && (entity instanceof TenantScoped tenantScopedEntity)) {
			if (tenantScopedEntity.getTenantId() != null && !tenantScopedEntity.getTenantId().equals(this.tenantScopeFactory.getInstance().getTenant())) return null;
		}
		return entity;
	}
	...
	public void reloadTenantFilters() throws InvalidApplicationException {
		...
		if (!this.tenantScopeFactory.getInstance().isSet()) return;

		if (!this.tenantScopeFactory.getInstance().isDefaultTenant()) {
			this.entityManager
					.unwrap(Session.class)
					.enableFilter(TenantScopedBaseEntity.TENANT_FILTER)
					.setParameter(TenantScopedBaseEntity.TENANT_FILTER_TENANT_PARAM, this.tenantScopeFactory.getInstance().getTenant().toString());
		} else {
			this.entityManager
					.unwrap(Session.class)
