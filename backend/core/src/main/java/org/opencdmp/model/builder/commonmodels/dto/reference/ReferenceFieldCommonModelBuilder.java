...
@Component("dto.ReferenceFieldCommonModelBuilder")
@Scope(value = ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class ReferenceFieldCommonModelBuilder extends BaseCommonModelBuilder<Field, ReferenceFieldModel> {
    ...
    @Override
    protected List<CommonModelBuilderItemResponse<Field, ReferenceFieldModel>> buildInternal(List<ReferenceFieldModel> data) throws MyApplicationException {
        this.logger.debug("building for {}", Optional.ofNullable(data).map(List::size).orElse(0));
        if (data == null || data.isEmpty()) return new ArrayList<>();

        List<CommonModelBuilderItemResponse<Field, ReferenceFieldModel>> models = new ArrayList<>();
        for (ReferenceFieldModel d : data) {
            Field m = new Field();
            m.setCode(d.getCode());
