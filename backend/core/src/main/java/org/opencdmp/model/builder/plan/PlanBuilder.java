...
public class PlanBuilder extends BaseBuilder<Plan, PlanEntity> {
    ...
    public PlanBuilder(ConventionService conventionService,
                       ...
                       BuilderFactory builderFactory, JsonHandlingService jsonHandlingService, XmlHandlingService xmlHandlingService, AuthorizationService authorizationService, AuthorizationContentResolverFactory authorizationContentResolverFactory, TenantScopeFactory tenantScopeFactory, CustomPolicyService customPolicyService, PlanStatusService planStatusService, UserScopeFactory userScopeFactory) {
    ...
    public PlanBuilder isPublic(boolean isPublic) {
    ...
    public List<Plan> build(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        Map<UUID, AffiliatedResource>  affiliatedResourceMap = authorizationFlags == null || authorizationFlags.isEmpty() ? null : this.authorizationContentResolverFactory.getInstance().plansAffiliation(data.stream().map(PlanEntity::getId).collect(Collectors.toList()));
        ...
        for (PlanEntity d : data) {
            ...
            if (!this.isPublic && fields.hasField(this.asIndexer(Plan._tenantId))) m.setTenantId(d.getTenantId());
            ...
            if (!this.isPublic && fields.hasField(this.asIndexer(Plan._versionStatus))) m.setVersionStatus(d.getVersionStatus());
            if (fields.hasField(this.asIndexer(Plan._publicAfter))) m.setPublicAfter(d.getPublicAfter());
            if (!this.isPublic && fields.hasField(this.asIndexer(Plan._hash))) m.setHash(this.hashValue(d.getUpdatedAt()));
            if (!this.isPublic && fields.hasField(this.asIndexer(Plan._belongsToCurrentTenant))) m.setBelongsToCurrentTenant(this.getBelongsToCurrentTenant(d, this.tenantScopeFactory.getInstance()));
    ...
    private Map<UUID, PlanStatus> collectPlanStatuses(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(PlanStatus._id))) {
            ...
            itemMap = this.builderFactory.builder(PlanStatusBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asForeignKey(q, clone, PlanStatus::getId);
        }
    ...
    private Map<UUID, List<PlanReference>> collectPlanReferences(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        itemMap = this.builderFactory.builder(PlanReferenceBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asMasterKey(query, clone, x -> x.getPlan().getId());
    ...
    private Map<UUID, List<EntityDoi>> collectEntityDois(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        itemMap = this.builderFactory.builder(EntityDoiBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asMasterKey(query, clone, EntityDoi::getEntityId);
    ...
    private Map<UUID, PlanBlueprint> collectPlanBlueprints(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        if (!fields.hasOtherField(this.asIndexer(PlanBlueprint._id))) {
            ...
            itemMap = this.builderFactory.builder(PlanBlueprintBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asForeignKey(q, clone, PlanBlueprint::getId);
        }
    ...
    private Map<UUID, List<PlanUser>> collectPlanUsers(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        if (this.isPublic) {
            ...
            List<PlanUserEntity> planUsers = this.queryFactory.query(PlanUserQuery.class).disableTracking().authorize(this.authorize).isActives(IsActive.Active).planIds(data.stream().map(PlanEntity::getId).distinct().collect(Collectors.toList())).collect();
    ...
    private Map<UUID, List<Description>> collectPlanDescriptions(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        itemMap = this.builderFactory.builder(DescriptionBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asMasterKey(query, clone, x -> x.getPlan().getId());
    ...
    private Map<UUID, List<Plan>> collectOtherPlanVersions(FieldSet fields, List<PlanEntity> data) throws MyApplicationException {
        ...
        itemMap = this.builderFactory.builder(PlanBuilder.class).authorize(this.authorize).isPublic(this.isPublic).asMasterKey(query, clone, Plan::getGroupId);
