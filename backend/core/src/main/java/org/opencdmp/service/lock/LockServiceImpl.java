...
public class LockServiceImpl implements LockService {
    ...
    public LockServiceImpl(
            ...
            ErrorThesaurusProperties errors, AuthorizationContentResolverFactory authorizationContentResolverFactory, LockProperties lockProperties) {
    ...
    public Lock persist(LockPersist model, FieldSet fields) throws MyForbiddenException, MyValidationException, MyApplicationException, MyNotFoundException, InvalidApplicationException {
        ...
        AffiliatedResource affiliatedResourcePlan = this.authorizationContentResolverFactory.getInstance().planAffiliation(model.getTarget());
        AffiliatedResource affiliatedResourceDescription = this.authorizationContentResolverFactory.getInstance().descriptionAffiliation(model.getTarget());
        AffiliatedResource affiliatedResourceDescriptionTemplate = this.authorizationContentResolverFactory.getInstance().descriptionTemplateAffiliation(model.getTarget());
        ...
        LockEntity data;
        if (isUpdate) {
            data = this.tenantEntityManagerFactory.getInstance().find(LockEntity.class, model.getId());
            ...
            if (!data.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) throw new MyApplicationException("Is not locked by that user");
            ...
            data.setLockedBy(this.userScopeFactory.getInstance().getUserId());
        }
    ...
    public LockStatus isLocked(UUID target, FieldSet fields) throws InvalidApplicationException {
        ...
        if (lock.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) lockStatus.setStatus(false);
        else {
    ...
    public void lock(UUID target, LockTargetType targetType) throws InvalidApplicationException {
        ...
        if (lock == null) {
            ...
            if (!lock.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) throw new MyApplicationException("Entity is already locked");
            this.touch(target);
        }
    ...
    public boolean checkLock(UUID target, LockTargetType targetType) throws InvalidApplicationException {
        ...
        if (lock == null) {
            ...
            if (!lock.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) return false;
            this.touch(target);
        }
    ...
    public void touch(UUID target) throws InvalidApplicationException {
        ...
        if (!lock.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) throw new MyApplicationException("Only the user who created that lock can touch it");
    ...
    public void unlock(UUID target) throws InvalidApplicationException {
        ...
        if (lock == null) throw new MyNotFoundException(this.messageSource.getMessage("General_ItemNotFound", new Object[]{target, Lock.class.getSimpleName()}, LocaleContextHolder.getLocale()));
        if (!lock.getLockedBy().equals(this.userScopeFactory.getInstance().getUserId())) {
    ...
    public void unlockMultipleTargets(UnlockMultipleTargetsPersist model) throws InvalidApplicationException {
        for (UUID target: model.getTargetIds()) {
            AffiliatedResource affiliatedResourcePlan = this.authorizationContentResolverFactory.getInstance().planAffiliation(target);
            AffiliatedResource affiliatedResourceDescription = this.authorizationContentResolverFactory.getInstance().descriptionAffiliation(target);
            AffiliatedResource affiliatedResourceDescriptionTemplate = this.authorizationContentResolverFactory.getInstance().descriptionTemplateAffiliation(target);
        ...
        if (locks.stream().filter(x -> x.getLockedBy().equals(this.userScopeFactory.getInstance().getUserIdSafe())).findFirst().orElse(null) == null) {
